%% AuE 893: Robust Predictive Conrtol HW04, Question 2,3
...
for i=1:N_mpc_all-10
    ...
    lb_u1  = input_constraint(2,i:i+N_mpc);% Lower bound for control input u1
    
    ub_x1  = state_constraint(1,i:i+N_mpc-1);% Upper bound for state x1
    ub_x2  = inf*ones(1,N_mpc);      % Upper bound for state x2
    ub_u1  = input_constraint(1,i:i+N_mpc);% Upper bound for control input u1
    ...
    XU_guess = opt_var;
    U  = opt_var((2*N_mpc)+1);
    W  = [(rand - 0.5)/5;(rand - 0.5)/5]; % W E [-0.1 0.1]
    mpc_sim = A*X_initial + B*U + W;
...
% Plot disturbance
figure()
plot(W_all(1,:),'r','LineWidth',2);hold on
plot(W_all(2,:),'b','LineWidth',2);grid on
