...
function [c,ceq] = nonlin_c(opt,X_ini,U_ini,A,B,time_step,Hp)
...
for i = 1:Hp
    ...
    % ref : openExample('optim/NonlinearConstraintsExample')
    c(i+3*Hp)   = (opt(i+3*Hp)- U_ini(1))^2 - (0.05/time_step)^2;
    c(i+1+4*Hp) = (opt(i+1+4*Hp)-U_ini(2))^2 - (0.05/time_step)^2;
