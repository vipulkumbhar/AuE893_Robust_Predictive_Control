%% AuE 893: Robust Predictive Conrtol HW03, Excercise 2.6 part a
...
for i = 1:mpc_steps
    ...
    lb_u2 = -inf*ones(1,N+1);   % Lower bound for control input u2
    
    ub_x1 = inf*ones(1,N);      % Upper bound for state x1
    ub_x2 = inf*ones(1,N);      % Upper bound for state x2
    ub_u1 = inf*ones(1,N+1);    % Upper bound for control input u1
    ub_u2 = inf*ones(1,N+1);    % Upper bound for control input u2
...
figure
plot(T,x(1,:),'b',T,x1_s,'--b',T,x(2,:),'r',T,x2_s,'--r','Linewidth',1)
xlabel('Simulation Time in seconds')
ylabel('States')
title('System State Trajectory')
legend('X1 - State','X1 - Set Point','X2 - State','X2 - Set Point')
grid on
...
legend('Control Input - u1','Control Input - u2')
grid on
...
figure
plot(T,y(1,:),'b',T,y1_s,'--b',T,y(2,:),'r',T,y2_s,'--r','Linewidth',1)
xlabel('Simulation Time in seconds')
ylabel('Outputs')
title('System Output Trajectory')
legend('Output 1','Output Set Point - 1', 'Output 2','Output Set Point - 2')
grid on